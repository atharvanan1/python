...
parser = argparse.ArgumentParser(prog='pcol', usage='pcol <number/s>', description='This script is used to display columns onto standard output from the table which shall be fed from standard input.')
parser.add_argument('integer', nargs='+', type=int, help='Enter the column number you wish to display', choices=range(1, 11))
...
def pcol():
    for row in range(len(store)):
        for column in args.integer:
            # Printing the data with right alignment. Noted that column-1 is for human based arguments,
            # where when user wants 1st column, he will put 1 as argument. However, as python indexes work
            # 1st column will be represented by 0.
            print("{0:>{width}}".format(big_store[row][column-1], width=width_return(column-1)), end = "  ", sep = "  ")
